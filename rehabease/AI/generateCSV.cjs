...
Object.keys(exerciseData).forEach(category => {
  exerciseData[category].forEach(exercise => {
    flattenedData.push({
      ...
      sets: exercise.sets,
      reps: exercise.reps,
      symptoms: exercise.symptoms.join(", "),
      patient_symptoms: exercise.patient_symptoms.join(", ")
    });
  });
});
...
// Write to CSV file
fs.writeFileSync('exercise_data.csv', csv);

console.log('CSV file has been generated: exercise_data.csv');
